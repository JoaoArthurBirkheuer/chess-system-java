...
public class ChessMatch {
	...
	private Piece makeMove(Position src, Position trgt) {
		...
		// castling kingside
		if(p instanceof King && trgt.getColumn() == src.getColumn() + 2) {
			Position srcR = new Position(src.getRow(), src.getColumn() + 3);
			Position trgtR = new Position(src.getRow(), src.getColumn() + 1);
			ChessPiece rook = (ChessPiece)board.removePiece(srcR);
			board.placePiece(rook, trgtR);
			rook.increaseMoveCount();
		}
		
		// castling queenside
		if(p instanceof King && trgt.getColumn() == src.getColumn() - 2) {
			Position srcR = new Position(src.getRow(), src.getColumn() - 4);
			Position trgtR = new Position(src.getRow(), src.getColumn() - 1);
			ChessPiece rook = (ChessPiece)board.removePiece(srcR);
			board.placePiece(rook, trgtR);
			rook.increaseMoveCount();
		...
		if(p instanceof Pawn) {
			if(src.getColumn() != trgt.getColumn() && capturedPiece == null) {
				Position pawnPosition;
				if(p.getColor() == Color.WHITE) {
					pawnPosition = new Position(trgt.getRow() + 1, trgt.getColumn());
				}
				else {
					pawnPosition = new Position(trgt.getRow() - 1, trgt.getColumn());
				}
	...
	private void undoMove(Position source, Position target, Piece capturedPiece)
	{
		...
		// castling kingside
				if(p instanceof King && target.getColumn() == source.getColumn() + 2) {
					...
					Position trgtR = new Position(source.getRow(), source.getColumn() + 1);
					ChessPiece rook = (ChessPiece)board.removePiece(trgtR);
	...
	private ChessPiece king(Color color) {
		...
		/// it is supposed to NEVER happen
	}
